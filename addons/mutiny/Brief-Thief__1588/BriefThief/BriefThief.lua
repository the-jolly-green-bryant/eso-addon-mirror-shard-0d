...
function bt:Initialize()
	bt.persistentSettings=ZO_SavedVars:NewAccountWide("BriefThiefVars", self.ver, nil, self.defaultPersistentSettings)
...
function bt:GetStolenItems()
	...
	local slotId for slotId = 0, bagCount, 1 do 
		if IsItemStolen(BAG_BACKPACK, slotId) then 
			...
			-- math that calculates percentage increase from haggle skill bonus (if bonus is not nil)
...
function bt:PersistentCommand(input, slash)
    ...
    if (slash == "loot" and bt.slash ~= "/loot") then return end
    if (slash == "thief" and bt.slash ~= "/thief") then return end
...
-- supresses auto firing on guard or fence
function BriefThief:PersistantHooks(event)
    if ((event == "guard") and (bt.guard)) then bt:Debrief() end
    if ((event == "fence") and (bt.fence)) then bt:Debrief() end
end
...
function BriefThief:CreateSettings()
    ...
    local panelData = {
	    type = "panel",
	    name = "Brief Thief",
	    displayName = bt.colors[bt.color].."Brief Thief",
	    author = "mutiny",
    ...
    local optionsData = {
        [1] = {
         ...
         [3] = {
            ...
            width = "half",
            getFunc = function() return bt.color end,
